...
# Correlation matrix as a heatmap for visualizing more than one static corr
# Use the following function: 
def corr_heatmap(df):
    idx = df.corr().sort_values("SPY", ascending=False).index
...
sns.boxplot(x=df1["HG=F"])
plt.title("Copper");


# Distribution chart 
fig = plt.figure(figsize=(5, 4)) 
ax = sns.distplot(df1["HG=F"])
xmin = df1["HG=F"].min()
xmax = df1["HG=F"].max()  
